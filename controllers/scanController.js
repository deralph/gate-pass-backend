...
export const processScan = async (req, res) => {
  try {
    ...
    if (lastScan) {
      isCurrentlyIn =
        lastScan.status === "approved" && lastScan.scanType === "in";
    }
  ...
  try {
    ...
    if (result === "approved") {
      await sendNotification(
        ...
        "vehicle_usage",
        "Vehicle Access",
        `Your vehicle ${scan.carId.plateNumber} was ${
          scan.scanType === "in" ? "granted entry" : "exited"
        } at the gate`,
        {
      ...
      await logActivity(scan.userId._id, `scan_${scan.scanType}`, {
        plateNumber: scan.carId.plateNumber,
      ...
      await logActivity(scan.userId._id, "scan_denied", {
        plateNumber: scan.carId.plateNumber,
        adminId: scan.adminId,
        scanId: scan._id,
        reason: reason,
    ...
    const scan1 = await Scan.findByIdAndUpdate(
      scanId,
      {
        scanType:
          scan.scanType === "in" && result === "approved" ? "out" : "in",
      },
      { new: true }
    )
      .populate("userId", "fullName email phoneNumber")
